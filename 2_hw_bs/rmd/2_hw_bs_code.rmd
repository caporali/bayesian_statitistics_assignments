...
Below are the autocorrelation functions of the parameters $\beta$ and $c$:
	\smallskip
	\scriptsize
...
That is,
\begin{align*}
& y_{1, j}, . . . , y_{n_j, j} | \theta_{j} , \sigma^2 `r simiid()` \normal{\theta_{j}}{\sigma^2}, j = 1, . . . , 8, \\
&  \theta_{1} , . . . , \theta_{8} | \mu, \tau^2  `r simiid()` \normal{\mu}{\tau^2}, \\
& \mu \sim \normal{\mu}{\gamma_{0}^2} , \quad 1/\tau^2 \sim Gamma(\eta_{0}/2, \eta_{0}\tau_{0}^2/2), \quad 1/\sigma^2 \sim Gamma(\nu_{0}/2, \nu_{0}\sigma_{0}^2/2)
\end{align*}

## Point a.
We implement a Gibbs sampling scheme that approximates the posterior distribution of ${\theta_{1:8}, \mu, \sigma^2, \tau^2}$. After a burning of $1000$, we run the Gibbs sampler long enough so that the effective sample sizes of all unknown parameters are greater than $1000$ (including the $\theta_i$'s).
\scriptsize
```{r}
set.seed(1)
...
\normalsize
\smallskip
As you can see, the chains span quite well and converge nicely and quite rapidly.
...
- $S_{eff}(\theta_{1:8}) = 
	\begin{aligned} 
		& `r paste(c("[", paste(round(matrix(effectiveSize(z), nrow = 1)[1:8], 0), collapse = ", "), "]"), collapse = "")` \\
	\end{aligned} \quad$.`r newline()`
	
which, compared to their length = `r s - burnin`, signifies a quite fast convergence.

## Point b.

We now proceed to compare the posterior densities of our parameters $\mu, \tau^2$ and $\sigma^2$ to their prior ones.

\scriptsize
```{r, fig.align = "center", out.width = "75%"}
library(invgamma)

#statistics for the MCMC
alpha = 0.05
...
\end{align*}
This parameter gives us an index of the relevance of the between-school variation (given that $\tau^2$ corresponds to the variance of the $\theta_i$'s), in relation to the within-school one (as $\sigma^2$ is the variance of the $Y_{i, j}$'s).
We start by observing that this index can only assume values in $[0, 1]$, given that both parameters are positive, and the $\tau^2$ contribute is the more relevant, the closer $R$ is to 1.
\smallskip
\scriptsize
```{r, fig.align = "center", out.width = "75%"}
...
sample_r %>% ggplot()+
  geom_density(aes(x = sample.r.prior, col = "prior"), fill = "red", alpha = 0.2, size = 0.7)+
  geom_density(aes(x = sample.r.post, col="posterior"), fill = "blue", alpha = 0.2, size = 0.7)+
  scale_color_manual(name = "Legend", values = c("prior" = "red", "posterior" = "blue"), 
    labels = c("prior distribution", "posterior distribution"))+
  ylab(" ") +
  xlab(" ") +
...
We observe that while in the prior the values are quite spread-out, which agrees with the independency of the two parameters, the posterior distribution in quite peaked and for a finite value, closer to 0 than to 1: this shows that while not being the dominant effect, we do not have enough evidence to consider the between-school variation negligible (as it is not close enough to zero).

## Point d.

Suppose we now observe a new school with school-specific parameter $\theta_9$, and suppose we make a new observation of this school $\tilde{Y_9}$. Let us compare them to those of school 7, in order to see weather or not they are higher than those of the latter: we will do so through Monte Carlo sampling.
\smallskip
\scriptsize
```{r}
...
- $\mathbb{P}[\; \tilde{Y}_{9} > \tilde{Y}_{7} \;] = `r mc_y`$. `r newline()`
...
mean_plt %>% ggplot(aes(x=post_means, y = ybar))+
  ...
  geom_text(aes(x = post_means[5], y = a_mu$statistics[1], label = paste("E[", "mu", "]"), colour = "orange"), vjust = -0.3)+
  ylab(expression(y[i])) +
...
The total sample mean is `r round(total.sample.mean, 2)` and the posterior expectation of the Grand Mean is `r round(a_mu$statistics[1], 2)`, which means that they only differ by `r round(abs(total.sample.mean - a_mu$statistics[1])/2 * 100, 2)`$\%$ of the former.`r newline()`
