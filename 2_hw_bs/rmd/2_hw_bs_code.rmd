...
- $\beta$ : 
	...
	acf_beta = acf(mcmc(beta), lag.max = 50, main = "")
	```
	\normalsize
- $c$ : 
	\smallskip
	...
	acf_c = acf(mcmc(c), lag.max = 50, main = "")
	```
\normalsize
- $Z_{1:25}$ (taken $i = 15$ as an example, so the autocorrelation function of $Z_{15}$) : 
	...
	acf_z_15 = acf(mcmc(z[, 15]), lag.max = 50, main = "")
	```
	\normalsize
...
Therefore we can obtain the 95%-quantile confidence regions of the posterior densities for each parameter.
