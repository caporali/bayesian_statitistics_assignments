...
```{r, fig.align = "center", out.width = "75%"}
  color_scheme_set("green")
	...
	mcmc_acf(as.data.frame(params_mcmc), pars = "beta", lags = 50)+
	  theme_bw()+
	  xlab(" ")
...
```{r, fig.align = "center", out.width = "75%"}
	...
	mcmc_acf(as.data.frame(params_mcmc), pars = "c", lags = 50, facet_args = list(nrow = 2, labeller = label_parsed))+
  theme_bw()
```
\normalsize
...
- $S_{eff}(\mu) = `r round(effectiveSize(mu.mcmc), 0)`\,$; \\
- $S_{eff}(\sigma^2) = `r round(effectiveSize(sigma2.mcmc), 0)`\,$; \\
- $S_{eff}(\tau^2) = `r round(effectiveSize(tau2.mcmc), 0)`\,$; \\
- $S_{eff}(\theta_{1:8}) = 
	\begin{aligned} 
		& `r paste(c("[", paste(round(matrix(effectiveSize(z), nrow = 1)[1:8], 0), collapse = ", "), "]"), collapse = "")` \\
	\end{aligned} \quad$.
which, compared to their length = `r s`, signifies a quite slow convergence.
...
The quantile-based $95\%$ posterior credible interval for $\mu, \tau^2$ and $\sigma^2$ are, respectively: \\
- $\mu$: [`r paste( a_mu$quantiles[1], ",", a_mu$quantiles[2])`] and the posterior mean is $\bar{\mu}$ = `r round(a_mu$statistics[1], 2)` 
- $\tau^2$: [ `r paste( a_tau$quantiles[1], ",", a_tau$quantiles[2])`] and the posterior mean is $\bar{\tau}^2$ = `r round(a_tau$statistics[1], 2)`
- $\sigma^2$: [ `r paste( a_sigma$quantiles[1], ",", a_sigma$quantiles[2])`] and the posterior mean $\bar{\sigma}^2$ = `r round(a_sigma$statistics[1], 2)`
...
-$\mathbb{P}[\theta_{9} > \theta_{7}] = `r mc_theta`$, \\
-$\mathbb{P}[\tilde{Y}_{9} > \tilde{Y}_{7}] = `r mc_y`$,

## Point e.

By plotting the sample averages ${\bar{y}_i}_{i = 1}^{8}$ against the posterior expectation values of the ${\theta_i}_{i = 1}^{8}$, we observe that they appear to have a linear relation.
\scriptsize
```{r}
library(ggrepel)
