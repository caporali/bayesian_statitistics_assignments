...
So we can write explicitly
    \begin{align*}
		...
		& = \exp{-\frac{1}{2} \frac{1}{\sigma_c^2} c^2} \prod_{{i = 1, \dots, n \given y_i = 0}} `r indicator()`_{(-\infty, z_i)}(c) \cdot \prod_{{i = 1, \dots, n \given y_i = 1}} `r indicator()`_{[z_i, +\infty)}(c) = \\
		& = \exp{-\frac{1}{2} \frac{1}{\sigma_c^2} c^2} `r indicator()`_{(-\infty, \, \min\left(z_i \given i \in \{1, \dots, n\}, y_i = 0\right))}(c) `r indicator()`_{[\max\left(z_i \given i \in \{1, \dots, n\}, y_i = 1\right), \, +\infty)}(c),
    \end{align*}
...
More compactly, defining 
	\begin{align*}
		a_n & \deq \max\left(z_i \given i \in \{1, \dots, n\}, y_i = 1\right) \text{ and } \\
		b_n & \deq \min\left(z_i \given i \in \{1, \dots, n\}, y_i = 0\right),
	\end{align*}
one has 
	\[
...
Then, let us observe that this conditional density is composed by the kernel of a gaussian (evaluated in $c$) multiplied by an indicator function (also evaluated in $c$), which constrains the domain to an interval (not necessarily limited, possibly $a_n = -\infty$ or $b_n = + \infty$). `r newline()`
