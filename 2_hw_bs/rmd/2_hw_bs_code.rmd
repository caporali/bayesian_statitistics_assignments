...
<!-- 
	edit: 
		non so come discutere del mixing anche perchè secondo me i risultati che abbiamo sono sbagliati.
		domani ne devo discutere con samuele e aggiusto di conseguenza
-->
...
The file \texttt{schools.RData} gives weekly hours spent on homework for students sampled from eight different schools. Obtain posterior distributions for the true means for the eight different schools using a hierarchical normal model
...
<!-- 
	edit: 
		1. sinceramente non saprei cosa concludere perchè non ho capito che legame c'è tra l'essere spread-out/peaked e la dipendenza di tau^2 e sigma^2.
		2. ho capito la strategia per determinare le densità, molto carina. ma lui in classe l'aveva mai fatto?
...
-->
We observe that while in the prior the values are quite peaked around $0.25$, the posterior distribution is quite spread-out, which agrees with the independency of the two parameters: \dots.
...
<!-- 
	edit:
		1. di nuovo, molto carina come soluzione, e stessa domanda che ti ho fatto sopra: lui lo aveva mai fatto?
		2. sei sicura di 
				rnorm(1, sample_theta_9[i], sqrt(sigma2[i] + tau2[i])) ?
			perchè secondo me ha più senso
				rnorm(1, sample_theta_9[i], sqrt(sigma2[i]))
-->
